...
class Httplib{
	...
	private $timeout = 3;
	private $cookie = '';
	private $useragent = 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.101 Safari/537.36';
	...
	public function set_timeout($seconds){
		if($seconds > @ini_get('max_execution_time')) @set_time_limit($seconds);
	...
	private function _process_response($ch = null){
		 if (is_resource($ch)) {
		 	...
		 	}else{ //使用代理的话获取的头部信息大小会有问题，所以代理时以内容大小截取
